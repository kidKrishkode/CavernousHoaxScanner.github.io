  ...
  try{
    setTimeout(()=>{
      if(pdf.imgSet.length!=0){
        ...
        let temp = `<div class="workspace blbg" style="background: #0000009e;" id="pdfPreview"><div class="pdfView"><header class="flx"><img src="../public/favicon.png" alt="load"/><div class="btn btn-outline-info dropdown-toggle" data-toggle="dropdown" onclick="invaild();">Re-Order</div><span style="cursor: pointer;" onclick="pdf.closeView();">&times;</span></header><div class="view">`;
...
IMGTOPDF.prototype.openView = function(){
  ...
  //     const targetIndex = Array.prototype.indexOf.call(pages, e.target);
  //     imgSet.splice(targetIndex, 0, imgSet.splice(index, 1)[0]);
  //     pdf.updatePages();
  //   });
  // });
  pages.forEach((page, index) => {
    ...
    page.addEventListener('drop', (e) => {
      ...
      [imgSet[index], imgSet[targetIndex]] = [imgSet[targetIndex], imgSet[index]];
      pdf.updatePages();
    });
  });  
}
...
IMGTOPDF.prototype.updatePages = function(){
  let imgSet = pdf.imgSet;
  const pages = document.querySelectorAll('.page');
  pages.forEach((page, index) => {
    page.querySelector('.imp').src = imgSet[index];
  });
}
